...
# Original source:
# https://github.com/SlinkyProject/containers/blob/945ac3492f8bf0a6b35a88a53b1a656d182a7cee/schedmd/slurm/25.11/ubuntu24.04/files/usr/local/bin/slurmd-entrypoint.sh
#
# Modifications made to the original file:
# - Replaced foreground daemon execution with exec into fakesystemd.sh.
# - Removed PID monitoring, shutdown handling (delegated to fakesystemd).
# - Removed creation of runtime directories.
# - Adjusted function definitions to not use the `function` keyword.
...
main() {
	...
	# Delegate daemon lifecycle to fakesystemd.sh (pidproxy).
	# shellcheck disable=SC2086
	exec /usr/sbin/fakesystemd.sh \
		/usr/sbin/slurmd --systemd $SLURMD_OPTIONS
}
